...
"""
import argparse, os, sys, csv, shutil, json, time
...
def find_candidates(root, old_bytes, include_exts, excludes, max_bytes):
...
def patch_textual(raw, old, token):
    if old not in raw:
        return raw
    return raw.replace(old, "${" + token + "}")

def worker(p, old, token, backup_dir, root_dir):
    path = Path(p)
    try:
    ...
    try:
        if backup_dir:
            backup_path = Path(backup_dir) / path.relative_to(Path(root_dir))
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as ex:
        futs = {ex.submit(worker, str(p), old, args.text_token, backup_dir, str(root)): str(p) for p in candidates}
        i = 0
