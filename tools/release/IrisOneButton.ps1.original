param(
  ...
  [switch]$NonInteractive = $false  # NEW: Skip all prompts
)
...
# Copy built files - check multiple locations including SvelteKit output
...
if (Test-Path $svelteKitClientDir) {
    Write-Host "   Found SvelteKit client output: $svelteKitClientDir" -ForegroundColor Gray
    Copy-Item -Path $svelteKitClientDir -Destination (Join-Path $releaseDir "dist") -Recurse
} elseif (Test-Path $rootDistDir) {
...
# Optional: Open the release folder (SKIP in non-interactive mode)
