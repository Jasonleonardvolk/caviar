...
# Smart detection for IrisOneButton script
...
# Use NonInteractive version if it exists, otherwise use original
if (Test-Path $IrisOneButtonNonInt) {
    $IrisOneButtonPs1 = $IrisOneButtonNonInt
    Write-Host "Using IrisOneButton_NonInteractive.ps1 (no prompts)" -ForegroundColor Green
} elseif (Test-Path $IrisOneButtonOrig) {
    $IrisOneButtonPs1 = $IrisOneButtonOrig
    Write-Host "Using IrisOneButton.ps1 (may have prompts - press N when asked)" -ForegroundColor Yellow
} else {
    Write-Host "ERROR: No IrisOneButton script found!" -ForegroundColor Red
    exit 1
}
...
$context = [ordered]@{
  ...
  cores    = $cores
  timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
  buildScript = Split-Path $IrisOneButtonPs1 -Leaf
}
...
if (Test-Path $IrisOneButtonPs1) {
  ...
  # Add NonInteractive flag if using the NonInteractive version
...
$md += ("- Build Script: {0}" -f $context.buildScript)
$md += ""
$md += "## Summary"
