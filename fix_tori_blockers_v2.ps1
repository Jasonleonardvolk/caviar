...
# Save Python script to file and run it
$pythonScript = @'
import os
import shutil
import json
...
$pythonScript | Out-File -FilePath "temp_fix_concepts.py" -Encoding UTF8
python temp_fix_concepts.py
Remove-Item "temp_fix_concepts.py"

Write-Host "`n🎉 Setup complete!" -ForegroundColor Green
Write-Host "`nThe syntax error in ingest_image.py has been fixed." -ForegroundColor Yellow
Write-Host "`nNow run: python enhanced_launcher.py" -ForegroundColor Cyan
