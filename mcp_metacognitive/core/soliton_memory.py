...
# Attempt real Rust extension - only import ConceptMesh
try:
    from concept_mesh_rs import ConceptMesh
    CONCEPT_MESH_AVAILABLE = True
    USING_RUST_WHEEL = True
    logging.info("🔧 Loaded real concept_mesh_rs extension")
    
    # Version check enhancement
    try:
        import concept_mesh_rs
        if hasattr(concept_mesh_rs, '__version__'):
            logging.info(f"📦 concept_mesh_rs version: {concept_mesh_rs.__version__}")
        else:
            logging.info("📦 concept_mesh_rs version: unknown")
    except:
        pass  # Version check is optional
        
except ImportError as e:
    logging.warning(f"⚠️ concept_mesh_rs not available: {e}")
    CONCEPT_MESH_AVAILABLE = False
    USING_RUST_WHEEL = False
    
    # Fallback ConceptMesh stub for development
    class ConceptMesh:
...
# Python-only types (always defined, not in Rust)
class MemoryEntry:
...
class SolitonMemoryClient:
    ...
    def __init__(self, api_url: Optional[str] = None):
        ...
        if CONCEPT_MESH_AVAILABLE:
            try:
                # Instantiate with the service URL (Rust #[new] expects exactly one arg)
                self.mesh = ConceptMesh(CONCEPT_MESH_URL)
                self.mesh_available = True
                logger.info("✅ Connected to Concept Mesh")
            except Exception as e:
                logger.error(f"❌ Failed to initialize Concept Mesh: {str(e)}")
                self.mesh_available = False
        else:
            self.mesh_available = False
            logger.warning("⚠️ Concept Mesh not available - some operations will be limited")
        ...
        logger.info(f"🌊 SolitonMemoryClient initialized")
        logger.info(f"🌊 Soliton API URL: {self.api_url}")
        logger.info(f"🌊 Concept Mesh: {'AVAILABLE' if self.mesh_available else 'UNAVAILABLE'}")
        logger.info(f"🌊 Local cache: {'ENABLED' if LOCAL_CACHE_ENABLED else 'DISABLED'}")
    ...
    async def initialize_user(self, user_id: str, options: Optional[Dict[str, Any]] = None) -> bool:
        ...
        """
        if not user_id:
            logger.error("❌ Cannot initialize user with empty ID")
            return False
        
        # Try using concept mesh directly if available
        if self.mesh_available and hasattr(self.mesh, 'initialize_user'):
            try:
                result = await self.mesh.initialize_user(user_id, **(options or {}))
                if result:
                    logger.info(f"✅ Initialized user {user_id} via Concept Mesh")
                    return True
            except Exception as e:
                logger.warning(f"⚠️ Failed to initialize user via Concept Mesh: {str(e)}")
        ...
        result = await self._call_api("init", "POST", data)
        
        if result.get("success"):
            logger.info(f"✅ Initialized user {user_id} via API")
            return True
        else:
            logger.error(f"❌ Failed to initialize user {user_id}: {result.get('error', 'Unknown error')}")
            return False
    ...
    async def store_memory(self,
        ...
        if self.mesh_available:
            try:
                ...
                logger.warning(f"⚠️ Failed to generate embedding via Concept Mesh: {str(e)}")
        ...
        if self.mesh_available:
            try:
                ...
                if result:
                    logger.info(f"✅ Stored memory {memory_id} for user {user_id} via Concept Mesh")
                ...
                logger.warning(f"⚠️ Failed to store memory via Concept Mesh: {str(e)}")
                # Fall through to API call
        ...
        if result.get("success"):
            logger.info(f"✅ Stored memory {memory_id} for user {user_id} via API")
            ...
            logger.error(f"❌ Failed to store memory {memory_id}: {result.get('error', 'Unknown error')}")
            return False
    ...
    async def find_related_memories(self,
        ...
        if not user_id or not query:
            logger.error("❌ Missing required parameters for finding related memories")
            return []
        ...
        if self.mesh_available:
            try:
                ...
                if memories:
                    logger.info(f"✅ Found {len(memories)} related memories via Concept Mesh")
                    return [memory.to_dict() for memory in memories]
            except Exception as e:
                logger.warning(f"⚠️ Failed to find related memories via Concept Mesh: {str(e)}")
                # Fall through to API call
        ...
        if result.get("success") and "memories" in result:
            logger.info(f"✅ Found {len(result['memories'])} related memories via API")
            return result["memories"]
        else:
            logger.error(f"❌ Failed to find related memories: {result.get('error', 'Unknown error')}")
            return []
    ...
    async def find_memories_by_tag(self,
        ...
        if not user_id or not tag:
            logger.error("❌ Missing required parameters for finding memories by tag")
            return []
        ...
        if self.local_cache is not None and user_id in self.local_cache:
            ...
            if memories:
                logger.info(f"✅ Found {len(memories)} memories with tag '{tag}' in local cache")
                return memories
        ...
        if self.mesh_available:
            try:
                ...
                if memories:
                    logger.info(f"✅ Found {len(memories)} memories with tag '{tag}' via Concept Mesh")
                    return [memory.to_dict() for memory in memories]
            except Exception as e:
                logger.warning(f"⚠️ Failed to find memories by tag via Concept Mesh: {str(e)}")
                # Fall through to API call
        ...
        if result.get("success") and "memories" in result:
            logger.info(f"✅ Found {len(result['memories'])} memories with tag '{tag}' via API")
            return result["memories"]
        else:
            logger.error(f"❌ Failed to find memories by tag: {result.get('error', 'Unknown error')}")
            return []
    ...
    async def get_memory(self,
        ...
        if not user_id or not memory_id:
            logger.error("❌ Missing required parameters for getting memory")
            return None
        
        # Try local cache first if enabled
        if self.local_cache is not None and user_id in self.local_cache and memory_id in self.local_cache[user_id]:
            logger.info(f"✅ Found memory {memory_id} in local cache")
        ...
        if self.mesh_available:
            try:
                ...
                if memory:
                    logger.info(f"✅ Retrieved memory {memory_id} via Concept Mesh")
                    return memory.to_dict()
            except Exception as e:
                logger.warning(f"⚠️ Failed to get memory via Concept Mesh: {str(e)}")
                # Fall through to API call
        ...
        if result.get("success") and "memory" in result:
            logger.info(f"✅ Retrieved memory {memory_id} via API")
            ...
            logger.error(f"❌ Failed to get memory {memory_id}: {result.get('error', 'Unknown error')}")
            return None
    ...
    async def get_user_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        """
        if not user_id:
            logger.error("❌ Missing user ID for getting stats")
            return {
                "totalMemories": 0,
                "activeWaves": 0,
                "averageStrength": 0,
        ...
        if self.mesh_available:
            try:
                ...
                if stats:
                    logger.info(f"✅ Retrieved stats for user {user_id} via Concept Mesh")
                    return stats
            except Exception as e:
                logger.warning(f"⚠️ Failed to get user stats via Concept Mesh: {str(e)}")
                # Fall through to API call
        ...
        if result.get("success") and "stats" in result:
            logger.info(f"✅ Retrieved stats for user {user_id} via API")
            return result["stats"]
        else:
            logger.error(f"❌ Failed to get stats for user {user_id}: {result.get('error', 'Unknown error')}")
    ...
    async def record_phase_change(self,
        ...
        if not user_id or not phase:
            logger.error("❌ Missing required parameters for recording phase change")
            return False
        ...
        if self.mesh_available:
            try:
                ...
                if result:
                    logger.info(f"✅ Recorded phase change {phase} for user {user_id} via Concept Mesh")
                    return True
            except Exception as e:
                logger.warning(f"⚠️ Failed to record phase change via Concept Mesh: {str(e)}")
                # Fall through to API call
        ...
        if result.get("success"):
            logger.info(f"✅ Recorded phase change {phase} for user {user_id} via API")
            return True
        else:
            logger.error(f"❌ Failed to record phase change: {result.get('error', 'Unknown error')}")
            return False
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        if self.mesh_available:
            try:
                ...
                if status:
                    logger.info(f"✅ Concept Mesh is operational")
                    return {
                        "status": "operational",
                        "engine": "concept_mesh",
                ...
                logger.warning(f"⚠️ Concept Mesh health check failed: {str(e)}")
                # Fall through to API call
        ...
        if "status" in result:
            logger.info(f"✅ Soliton API health check: {result['status']}")
            return result
        else:
            logger.error(f"❌ Soliton API health check failed: {result.get('error', 'Unknown error')}")
            return {
                "status": "error",
                "engine": "unknown",
    ...
    async def close(self):
        ...
        logger.info("🌊 SolitonMemoryClient closed")
...
async def verify_connectivity():
    ...
    logger.info(f"🌊 Soliton Memory Connectivity Verification:")
    logger.info(f"🌊 API Available: {api_available}")
    logger.info(f"🌊 Chat Sources: {chat_count} memories")
    logger.info(f"🌊 Ingest Sources: {ingest_count} memories")
    logger.info(f"🌊 Other Sources: {other_count} memories")
    logger.info(f"🌊 All Sources Active: {all_sources_active}")
