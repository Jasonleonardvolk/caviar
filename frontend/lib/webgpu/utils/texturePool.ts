// texturePool.ts - Centralized texture allocation with memory pressure handling
export interface TexturePoolConfig {
  maxBudgetMB?: number;
  onBudgetExceeded?: () => void;
  onAllocationFailed?: (error: Error) => void;
}

export class TexturePool {
  private device: GPUDevice;
  private allocatedTextures: Map<GPUTexture, number> = new Map(); // texture -> size in bytes
  private totalAllocated: number = 0;
  private maxBudget: number;
  private _onBudgetExceeded: () => void;
  private onAllocationFailed: (error: Error) => void;
  private allocationFailures: number = 0;
  ...
  create(desc: GPUTextureDescriptor): GPUTexture {
    ...
    try {
      ...
      this.onAllocationFailed(e as Error);
  ...
  private calculateTextureSize(desc: GPUTextureDescriptor): number {
  ...
  // Public setter for budget exceeded callback
  set onBudgetExceeded(callback: () => void) {
    this._onBudgetExceeded = callback;
  }
...
let globalPool: TexturePool | null = null;

export function initTexturePool(device: GPUDevice, config?: TexturePoolConfig): TexturePool {
  if (!globalPool) {
...
export function getTexturePool(): TexturePool {
  if (!globalPool) {
