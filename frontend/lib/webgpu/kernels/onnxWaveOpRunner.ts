...
class SessionCache {
    ...
    async get(modelPath: string | Uint8Array, options?: InferenceSession.SessionOptions): Promise<CachedSession> {
    ...
    release(modelPath: string | Uint8Array, options?: InferenceSession.SessionOptions): void {
        const key = this.getCacheKey(modelPath, options);
        const session = this.cache.get(key);
    ...
    private getCacheKey(modelPath: string | Uint8Array, options?: InferenceSession.SessionOptions): string {
    ...
    private async createSession(
        ...
        const modelData = typeof modelPath === 'string'
            ...
            : modelPath;
        const session = await InferenceSession.create(modelData, options);
...
export class OnnxWaveOpRunner {
    ...
    private outputTensor: Tensor | null = null;
    ...
    private async selectBackend(): Promise<'webgpu' | 'wasm'> {
        ...
        try {
            ...
            if (this.device && 'gpu' in navigator) {
                ...
                const testSession = await InferenceSession.create(
                    modelData,
                    {
                        executionProviders: [{
                            name: 'webgpu',
    ...
    private async setupStandardTensors(): Promise<void> {
        ...
        this.inputTensor = new Tensor('float32', data, shape);
