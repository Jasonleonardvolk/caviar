...
function determinePerformanceTier(limits: GPUSupportedLimits): 'high' | 'medium' | 'low' {
...
function getSafeLimits(
  ...
  if (platform.isIOS && platform.isSafari) {
    ...
    requested.maxTextureDimension2D = Math.min(
      limits.maxTextureDimension2D || 8192,
      8192
    );
    requested.maxBufferSize = Math.min(
      limits.maxBufferSize || 268435456,
      268435456 // 256MB max for iOS
    );
    requested.maxComputeWorkgroupSizeX = Math.min(
      limits.maxComputeWorkgroupSizeX || 256,
      256
    );
    requested.maxComputeWorkgroupSizeY = Math.min(
      limits.maxComputeWorkgroupSizeY || 256,
      256
    );
    requested.maxComputeInvocationsPerWorkgroup = Math.min(
    ...
    requested.maxBufferSize = Math.min(
      limits.maxBufferSize || 134217728,
      134217728 // 128MB for mobile
    );
  }
  ...
  if (options?.requiredLimits) {
    Object.entries(options.requiredLimits).forEach(([key, value]) => {
      const adapterLimit = (limits as any)[key];
...
export async function initWebGPU(options?: InitOptions): Promise<{ 
  device: GPUDevice; 
  caps: Capabilities;
  ...
  if (!options?.forceFallback) {
    ...
    if (hasDepthClipControl) requiredFeatures.push('depth-clip-control');
    if (hasTimestamp && !platform.isIOS) { // iOS doesn't like timestamp queries
      requiredFeatures.push('timestamp-query');
    }
    if (hasIndirectFirstInstance) requiredFeatures.push('indirect-first-instance');
    if (hasDepth32FloatStencil8) requiredFeatures.push('depth32float-stencil8');
  ...
  console.log('[WebGPU] Device initialized:', {
    ...
    limits: Object.fromEntries(
      Object.entries(limits).slice(0, 5) // Just log first few limits
    ),
    platform,
    fallbackMode,
    adapterInfo
