...
export class WASMFallbackRenderer {
  ...
  private viewIndices = { h: 4, v: 0 }; // Center view by default
  private quiltSize = { width: 2048, height: 2048 };
  private viewCount = { h: 9, v: 1 };
  private deviceTier: string;
  ...
  private async loadWASMModule(): Promise<void> {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        this.wasmModule = {
          renderQuiltView: (h: number, v: number) => {
  ...
  private simulateQuiltView(h: number, v: number): ImageData {
  ...
  private startRenderLoop(): void {
    const render = () => {
      ...
      // Render current view
      if (this.wasmModule) {
        const viewData = this.wasmModule.renderQuiltView(
          this.viewIndices.h,
          this.viewIndices.v
        );
  ...
  public updateView(h: number, v: number): void {
...
export async function init(): Promise<void> {
  ...
  window.addEventListener('controlUpdate', (event: CustomEvent) => {
  ...
  (window as any).wasmRenderer = renderer;
