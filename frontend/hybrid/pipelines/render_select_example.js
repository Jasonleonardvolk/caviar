...
export class RenderPipeline {
    constructor() {
        this.phaseTuner = getPhaseTuner();
        this.phaseConfig = getPhaseConfig();
        this.deviceKey = 'default';
    }
    async init() {
...
Test with: ?phase=tv&tv=0.08&maxc=0.25&auto=1&metrics=1
     ...
     */
    render(encoder, width, height, timestamp) {
    ...
    async detectDeviceTier() {
        ...
        if (vendor.includes('Apple')) {
            if (device.includes('M2'))
                return 'mac_m2';
            if (device.includes('A17'))
                return 'ios_a17';
        }
        return 'default';
    }
...
// ============================================
export async function simpleIntegration(encoder, reBuf, imBuf, width, height) {
    // Get singleton tuner
    const tuner = getPhaseTuner();
    // Initialize once (do this at app startup)
    static let initialized = false;
    if (!initialized) {
