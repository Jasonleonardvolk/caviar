...
export function mountHeadTrackingSettings({ manager }) {
    const root = el('div');
    ...
    const title = el('div');
    title.textContent = 'Head Tracking';
    title.style.fontWeight = '700';
    sec0.append(title);
    // Enable toggle
    const sec1 = el('div', 'section');
    const rowToggle = el('div', 'row');
    const labToggle = el('div');
    labToggle.textContent = 'Enabled';
    const toggle = document.createElement('input');
    toggle.type = 'checkbox';
    toggle.className = 'toggle';
    toggle.setAttribute('aria-label', 'Enable head tracking');
    rowToggle.append(labToggle, toggle);
    const subToggle = el('div', 'subtle');
    subToggle.textContent = 'Uses FaceID/WebXR/tilt when available; falls back to mouse.';
    sec1.append(rowToggle, subToggle);
    // Provider
    const sec2 = el('div', 'section');
    const rowProv = el('div', 'row');
    const labProv = el('div');
    labProv.textContent = 'Provider';
    const select = document.createElement('select');
    select.setAttribute('aria-label', 'Head tracking provider');
    ...
    const subProv = el('div', 'subtle');
    subProv.textContent = 'Auto prefers FaceID/WebXR when supported.';
    sec2.append(rowProv, subProv);
    // Tuning
    const sec3 = el('div', 'section');
    const h2 = el('div');
    h2.textContent = 'Tuning';
    ...
    const nAlpha = mkNum('alpha', 'alpha');
    const nBeta = mkNum('beta', 'beta');
    const nGamma = mkNum('gamma', 'gamma');
    const nMCut = mkNum('minCutoff', 'euroMinCutoff');
    const nEBeta = mkNum('euroBeta', 'euroBeta');
    const nDCut = mkNum('dCutoff', 'euroDerivCutoff');
    [nAlpha, nBeta, nGamma, nMCut, nEBeta, nDCut].forEach(x => grid.appendChild(x.parentElement));
    const btns = el('div', 'btnrow');
    const btnApply = el('button', 'btn');
    btnApply.textContent = 'Apply';
    const btnReset = el('button', 'btn');
    ...
    select.onchange = async () => { await manager.setProviderPref(select.value); };
    btnApply.onclick = () => {
