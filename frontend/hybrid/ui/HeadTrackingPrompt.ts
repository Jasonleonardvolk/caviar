// ${IRIS_ROOT}\frontend\hybrid\ui\HeadTrackingPrompt.ts
type Options = {
  onEnable: () => Promise<boolean>; // return true if enhanced tracking started
  onDismiss?: () => void;
  storageKey?: string;              // override LS key
...
export function mountHeadTrackingPrompt(opts: Options) {
  ...
  if (val) {
    try {
      const state = JSON.parse(val) as { status: 'accepted'|'dismissed'; ts: number };
      if (state.status === 'accepted') return () => {};
      if (state.status === 'dismissed' && state.ts > daysAgo(snoozeDays)) return () => {};
  ...
  const hide = () => {
    ...
    setTimeout(() => { if (root.isConnected) root.remove(); }, 200);
  };
  ...
  const enableBtn = wrap.querySelector('.enable') as HTMLButtonElement;
  const dismissBtn = wrap.querySelector('.dismiss') as HTMLButtonElement;
  ...
  enableBtn.onclick = async () => {
    try {
      ...
      if (ok) {
        ...
        hide(); toast('Head tracking enabled');
      } else {
  ...
  if (opts.autoShow ?? true) window.addEventListener('pointerdown', revealOnce, true);
