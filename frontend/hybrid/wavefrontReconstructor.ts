...
export class WavefrontReconstructor {
    ...
    private dim: number;               // width/height of the square wavefield grid
    private phaseOcclusionPipeline: GPUComputePipeline|null = null;
    private multiDepthPipeline: GPUComputePipeline|null = null;
    private bindGroupOcclusion: GPUBindGroup|null = null;
    private bindGroupMultiDepth: GPUBindGroup|null = null;
    // GPU buffers for wavefield and occlusion data
    private waveBufferIn: GPUBuffer|null = null;
    private waveBufferMid: GPUBuffer|null = null;
    private waveBufferOut: GPUBuffer|null = null;
    private occlusionBuffer: GPUBuffer|null = null;
    // GPU uniform buffers for shader parameters
    private occlusionParamsBuffer: GPUBuffer|null = null;
    private multiDepthParamsBuffer: GPUBuffer|null = null;
    ...
    private async initGPUPipeline() {
        ...
        const phaseOcclusionCode: string = /* wgsl */`
        ...
        `;
        const multiDepthCode: string = /* wgsl */`
     ...
     */
    public async reconstructWavefront(personaEmbedding: number[], conceptMesh: any, audioData?: Float32Array, userState?: {
        emotion?: number, proximity?: number, gazeX?: number, gazeY?: number, cognitiveFactor?: number
        ...
        if (conceptComplexity > 100) conceptComplexity = 100;
        ...
        if (this.useWebGPU && this.device) {
            ...
            // Recalculate waveBufferSize (N * 2 floats * 4 bytes per float)
            const waveBufferSize = N * 2 * 4;
            ...
            if (occ) {
                ...
                for (let y = 0; y < height; y++) {
                    for (let x = 0; x < width; x++) {
                        ...
                        let sum = occ[i];
                        let count = 1;
                        if (x + 1 < width)  { sum += occ[i+1]; count++; }
                        if (x > 0)         { sum += occ[i-1]; count++; }
                        if (y + 1 < height) { sum += occ[i+width]; count++; }
                        if (y > 0)         { sum += occ[i-width]; count++; }
            ...
            const layerPhaseOffsets: number[] = [];
