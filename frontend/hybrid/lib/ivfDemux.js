 ...
 */
export function parseIVFHeader(buffer) {
    ...
    const codec = String.fromCharCode(view.getUint8(8), view.getUint8(9), view.getUint8(10), view.getUint8(11));
    // Read dimensions
 ...
 */
export function* parseIVFFrames(buffer, header) {
    const view = new DataView(buffer);
 ...
 */
export async function loadIVF(url) {
 ...
 */
export function isValidIVF(buffer) {
    if (buffer.byteLength < 32) {
        return false;
    }
    const view = new DataView(buffer);
 ...
 */
export function getCodecName(fourcc) {
    const codecs = {
        'AV01': 'AV1',
        'VP80': 'VP8',
        'VP90': 'VP9',
