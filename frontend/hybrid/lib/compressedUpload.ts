...
const BLOCK_FORMATS: Record<string, BlockInfo> = {
 ...
 */
export function getGPUFormat(
  format: 'astc' | 'bc7' | 'etc2' | 'rgba',
  srgb: boolean = false
): { format: GPUTextureFormat; blockFormat?: keyof typeof BLOCK_FORMATS } {
  switch (format) {
    case 'astc':
      return {
 ...
 */
export function getSupportedFormats(adapter: GPUAdapter): Set<'astc' | 'bc7' | 'etc2'> {
  const supported = new Set<'astc' | 'bc7' | 'etc2'>();
