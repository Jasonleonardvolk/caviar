// ${IRIS_ROOT}\frontend\hybrid\lib\sensors\selectBestSensorProvider.ts
...
export async function selectBestSensorProvider(canvas: HTMLCanvasElement): Promise<ISensorProvider> {
  // Preference order: FaceID depth (iOS) > WebXR (inline) > DeviceOrientation > Mouse
  const candidates: ISensorProvider[] = [
  ...
  for (const c of candidates) {
    try { if (await c.isSupported()) return c; } catch { /* ignore */ }
  }
