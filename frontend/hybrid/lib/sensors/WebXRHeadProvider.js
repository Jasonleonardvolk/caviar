export class WebXRHeadProvider {
    constructor() {
        this.name = 'WebXR(inline)';
        this.onPose = null;
        this.session = null;
        this.refSpace = null;
        this.running = false;
    ...
    async isSupported() {
        ...
        if (!anyNav?.xr?.isSessionSupported)
            return false;
        try {
            return await anyNav.xr.isSessionSupported('inline');
        }
        catch {
            return false;
        }
    ...
    async start(onPose) {
        ...
        const onXRFrame = (_time, frame) => {
            if (!this.running || !this.session || !this.refSpace)
                return;
            ...
            if (pose) {
                ...
                const poseOut = { p: [p.x, p.y, p.z], r: [yaw, pitch, roll], confidence: 0.9, t: now };
                this.onPose?.(poseOut);
            }
