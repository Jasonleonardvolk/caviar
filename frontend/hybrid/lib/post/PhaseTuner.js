...
export class PhaseTuner {
    constructor() {
        this.config = getPhaseConfig();
        this.frameCount = 0;
        this.metricsInterval = 60;
        this.lastMetrics = {
            method: 'off',
            avgCorrection: 0,
            maxCorrection: 0,
    ...
    async init(deviceKey) {
        ...
        if (deviceKey) {
            try {
                ...
                await this.lut.init();
                await this.lut.loadFromUrls(`/corrections/${deviceKey}/phase_offset_f32.bin`, `/corrections/${deviceKey}/meta.json`);
     ...
     */
    apply(encoder, reBuf, imBuf, width, height, maskBuf) {
     ...
     */
    updateMetrics(method, frameTime) {
        this.frameCount++;
        // Update last metrics
     ...
     */
    async analyzeWavefield(reBuf, imBuf, width, height) {
