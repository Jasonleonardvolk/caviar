...
export class ApplyPhaseLUT {
    ...
    async loadFromUrls(binUrl, metaUrl) {
        const [binRes, metaRes] = await Promise.all([
            fetch(binUrl),
            metaUrl ? fetch(metaUrl) : Promise.resolve(undefined),
        ]);
        if (!binRes.ok)
            throw new Error(`Dphi bin fetch failed: ${binRes.status} ${binUrl}`);
        const bin = new Float32Array(await binRes.arrayBuffer());
        let meta;
        if (metaUrl) {
            if (!metaRes.ok)
                throw new Error(`Dphi meta fetch failed: ${metaRes.status} ${metaUrl}`);
        ...
        if (meta?.shape_hw) {
            ...
            if (s * s !== n)
                throw new Error(`Cannot infer LUT shape (len=${n}); provide meta.json`);
            this.lutH = s;
            this.lutW = s;
        }
    ...
    run(encoder, reBuf, imBuf, width, height, opts) {
        if (!this.dphiBuf)
            throw new Error("Dphi LUT not loaded");
