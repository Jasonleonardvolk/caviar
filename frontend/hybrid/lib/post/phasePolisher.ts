...
export type PhasePolisherParams = {
  width: number; 
  height: number;
  tvLambda?: number;        // default 0.08
  maxCorrection?: number;   // default 0.25
  useMask?: boolean;
};
...
export class PhasePolisher {
  ...
  run(encoder: GPUCommandEncoder,
      reBuf: GPUBuffer, imBuf: GPUBuffer,
      params: PhasePolisherParams,
      maskBuf?: GPUBuffer) {
    ...
    u32v[0] = width;  u32v[1] = height;
    f32v[2] = tv;     f32v[3] = maxc;
    ...
    device.queue.writeBuffer(this.paramBuf, 0, paramArray.slice(.buffer));
