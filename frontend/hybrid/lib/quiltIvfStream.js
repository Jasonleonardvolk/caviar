...
export class AV1StreamPlayer {
     ...
     * Initialize decoder
     */
    async initDecoder(width, height, codec) {
 ...
 */
async function loadFile(url, canvas) {
    // Load IVF file
    ...
    // Initialize decoder
    await this.initDecoder(this.reader.width, this.reader.height, this.reader.codec);
 ...
 */
export async function startIvfStream(ivfUrl, canvas, options = {}) {
 ...
 */
export async function createVideoFromIVF(ivfUrl) {
...
export class WebGPUVideoTexture {
    ...
    updateFromFrame(frame) {
        ...
        this.device.queue.copyExternalImageToTexture({ source: frame }, { texture: this.texture }, {
            width: frame.displayWidth,
            height: frame.displayHeight
        });
        return this.texture;
