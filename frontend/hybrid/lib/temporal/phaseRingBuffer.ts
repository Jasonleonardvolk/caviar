...
export class PhaseRingBuffer {
  ...
  private accumulatePipeline: GPUComputePipeline | null = null;
  ...
  constructor(device: GPUDevice, config: RingBufferConfig) {
    this.device = device;
  ...
  async accumulate(
    viewDelta: { dx: number; dy: number },
    z: number,
    wavelength: number = 0.000532 // Green laser default
  ): Promise<GPUTexture> {
