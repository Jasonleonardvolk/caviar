...
export class HeadPosePredictor {
    constructor(opts = {}) {
        this.euroP = [new OneEuro(), new OneEuro(), new OneEuro()];
        this.euroR = [new OneEuro(1.0, 0.05), new OneEuro(1.0, 0.05), new OneEuro(1.0, 0.05)];
        this.x = [0, 0, 0];
        this.v = [0, 0, 0];
        this.a = [0, 0, 0];
        this.rpy = [0, 0, 0];
        ...
        this.lastPrediction = { p: [0, 0, 0], r: [0, 0, 0], confidence: 0, t: 0, dtAhead: 0 };
        this.opts = {
            mode: opts.mode ?? 'ab',
    ...
    update(meas, dtAheadMs) {
        if (!this.initialized)
            this.reset(meas);
        const dtMs = Math.min(this.opts.maxDt, Math.max(1e-3, meas.t - this.tPrev));
        this.tPrev = meas.t;
        const dt = dtMs / 1000;
        // 1 Euro denoise
        const mP = [0, 0, 0];
        ...
        if (meas.confidence < this.opts.confidenceFloor) {
            this.v = this.v.map(v => v * 0.9); // freeze/decay velocity
            this.rpy = mR;
            return this.predict(dtAheadMs);
        }
        // alpha-beta[-gamma]
        if (this.opts.mode === 'ab') {
            const xbar = [this.x[0] + this.v[0] * dt, this.x[1] + this.v[1] * dt, this.x[2] + this.v[2] * dt];
            const r = [mP[0] - xbar[0], mP[1] - xbar[1], mP[2] - xbar[2]];
            ...
            const vbar = [this.v[0] + this.a[0] * dt, this.v[1] + this.a[1] * dt, this.v[2] + this.a[2] * dt];
            const r = [mP[0] - xbar[0], mP[1] - xbar[1], mP[2] - xbar[2]];
            this.x = [xbar[0] + this.opts.alpha * r[0], xbar[1] + this.opts.alpha * r[1], xbar[2] + this.opts.alpha * r[2]];
    ...
    peek() { return this.lastPrediction; }
    setParams(p) {
