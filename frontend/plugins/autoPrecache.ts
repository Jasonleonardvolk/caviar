...
export interface AutoPrecacheOptions {
  include?: string[];      // Glob patterns relative to project root
  extra?: string[];        // Extra paths to include
  manifestName?: string;   // Output filename
 ...
 */
export default function autoPrecache(opts: AutoPrecacheOptions = {}): Plugin {
 ...
 */
export function afterBuildPrecache(opts: AutoPrecacheOptions = {}): Plugin {
  const manifestName = opts.manifestName ?? 'precache.manifest.json';
  const includeExt = [
