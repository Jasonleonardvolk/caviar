# serve_embeddings_production_v2.py - Compatible with sentence-transformers 2.5.1
import os
import asyncio
import logging
import time
...
batch_size = 32  # Global variable for batch size
...
async def lifespan(app: FastAPI):
    ...
    # Load model - compatible with sentence-transformers 2.5.1
    ...
    cache_dir = os.getenv("TORI_EMBED_CACHE", "./emb_cache")  # Use local dir for Windows
...
async def embedding_stats():
    ...
    return {
        ...
        "concurrency": {
            "semaphore_capacity": batch_size,
            "current_requests": f"{embedding_semaphore._value if embedding_semaphore else 0}/{batch_size}"
        }
    }
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "serve_embeddings_production_v2:app",
        host="0.0.0.0",
        port=port,
        workers=workers,
        reload=False,
